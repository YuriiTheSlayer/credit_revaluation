"""Экран дашборда: загрузка файлов, переключатель банка, фильтры, KPI,
таблица SKU, графики и экспорт в Excel. Вся тяжёлая работа (парсинг, экспорт)
выполняется в отдельном потоке, UI не блокируется.
"""
...
class Dashboard:
    ...
    def __init__(self, page: ft.Page):
        ...
        self.zone_competitors, self.status_competitors = w.upload_zone(
            "CSV конкурентов", "Кликните, чтобы выбрать файл (cp1251/UTF-8)",
            ft.Icons.TABLE_VIEW,
            lambda _e: self.pick_competitors.pick_files(
                allow_multiple=False, allowed_extensions=["csv", "txt"]),
        )
        self.zone_sales, self.status_sales = w.upload_zone(
            "Файл продаж", "Опционально: CSV или XLSX с колонками КодТовара и Оборот",
            ft.Icons.SHOPPING_CART,
            lambda _e: self.pick_sales.pick_files(
                allow_multiple=False, allowed_extensions=["csv", "txt", "xlsx", "xlsm"]),
        )
        ...
        self.f_assort_global = w.MultiSelect(page, "ТипАссортиментаОбщий", self._filters_changed)
        self.f_brand = w.MultiSelect(page, "Бренд", self._filters_changed)
        self.search_field = ft.TextField(
            hint_text="Поиск: название или код товара…",
            prefix_icon=ft.Icons.SEARCH, dense=True, width=320,
            on_change=self._search_changed, disabled=True,
        )
        self.reset_filters_btn = ft.TextButton(
            "Сбросить фильтры", icon=ft.Icons.FILTER_ALT_OFF,
            on_click=self._reset_filters, disabled=True,
        )

        self.kpi_row = ft.Row(wrap=True, spacing=10)
        self.chart_terms_host = ft.Container(padding=8)
        self.chart_dev_host = ft.Container(padding=8)
        self.chart_legend = ft.Container()
        self.charts_tabs = ft.Tabs(
            tabs=[
                ft.Tab(text="Средний срок по категориям", content=self.chart_terms_host),
                ft.Tab(text="Распределение отклонений", content=self.chart_dev_host),
            ],
        ...
        self.warnings_tile = ft.ExpansionTile(
            title=ft.Text("Предупреждения", size=13), controls=[], visible=False,
            initially_expanded=False,
        )
        self.empty_hint = ft.Container(
            ft.Column(
                [
                    ft.Icon(ft.Icons.UPLOAD_FILE, size=48, color=w.MUTED),
                    ft.Text("Загрузите CSV-выгрузку по конкурентам, чтобы построить дашборд",
                            color=w.MUTED),
                ],
    ...
    def _load_competitors(self, path: str, name: str) -> None:
        try:
            ...
            self.status_competitors.value = (
                f"{name} · {data.encoding} · SKU: {data.sku_count} · "
    ...
    def _load_sales(self, path: str, name: str) -> None:
        try:
            ...
            self.status_sales.value = (
                f"{name} · строк: {len(sales.df)} · оборот: {w.fmt_money(sales.total)}"
            )
    ...
    def _after_dataset_change(self) -> None:
        ...
        self.warnings_tile.title = ft.Text(f"Предупреждения ({len(all_warnings)})", size=13)
        self.warnings_tile.controls = [
    ...
    def _refresh_kpi(self, filtered, value_col, names, pay_cols) -> None:
        ...
        if has_comp.any():
            ...
            cards.append(w.kpi_card(
                ...
                f"среди {int(has_comp.sum())} SKU с данными конкурентов",
    ...
    def _refresh_charts(self, filtered, value_col, names, pay_cols) -> None:
        self.charts_tabs.visible = len(filtered) > 0
        if not len(filtered):
            return
        ...
        chart = ft.BarChart(
            ...
            horizontal_grid_lines=ft.ChartGridLines(interval=3, color=ft.Colors.GREY_300),
        ...
        if len(dist):
            ...
            for gi, (dev, count) in enumerate(dist.items()):
                color = w.BAD if dev < 0 else (w.GOOD if dev > 0 else ft.Colors.GREY_500)
            ...
            self.chart_dev_host.content = ft.Column([
                ...
                ft.Container(ft.BarChart(
                    ...
                    horizontal_grid_lines=ft.ChartGridLines(
                        interval=1, color=ft.Colors.GREY_300),
                    expand=True,
                ), height=300),
            ])
        else:
            self.chart_dev_host.content = ft.Text("Нет данных по конкурентам", color=w.MUTED)

    def _refresh_table(self, filtered, value_col, names) -> None:
        self.table.visible = len(filtered) > 0
        ...
        self.table_pager.controls[1].value = (
            f"{start + 1}–{min(start + self.PAGE_SIZE, len(df))} из {len(df)} · "
    ...
    def _export_clicked(self, _e) -> None:
        ...
        if not self.dataset.has_sales:
            self._toast("Файл продаж не загружен — экспортируется только лист "
                        "«Средний срок на стоимость».")
